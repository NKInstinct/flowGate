...
#cowplot::ggdraw() + cowplot::draw_image("gaterlogo.svg")
```
...
flowGate was developed to fill in this missing ability for manual, GUI-based gating
in R, finally enabling a familiar cytometry analysis pipeline completely within
R. This vignette will demonstrate the flowGate function within the context of a
...
Prior to using flowGate, it must first be installed on your system. Because it is
in active development and not yet submitted to BioConductor, you will need to
...
1. Install RStudio (https://rstudio.com). flowGate uses the Shiny package to make
interactive gating possible, which tends to work best from inside the RStudio
IDE. You don't strictly need to use RStudio to make this work, but this vignette
is assuming that you are, and if you don't have a reason not to use RStudio, I
recommend that you do at least while you are working through this vignette. 
...
4. Install BioConductor dependencies (see code below). Once flowGate is submitted
to BioConductor, this step won't be necessary, but for the meantime, the code
we will use to install flowGate from github doesn't know how to automatically
install dependencies that are hosted on BioConductor, so we need to do it
ourselves first. Run the following code on your computer before proceeding:
...
shipped with flowGate. However, when you are ready to read your own flow data into
...
which is automatically installed with flowGate for visualization purposes. Since we
...
add `regate = TRUE` to the command so flowGate knows to delete this Leukocytes gate
...
__NB:__ the ggcyto package insists on "colour" as the only spelling of the word. Trying to set the "color" of your gate will result in hours of fruitless debugging.
...
The authors thank Dr. Meromit Singer and the HMS computing core for help getting
our lab started in applying bioinformatics methods. The original code for flowGate
was based on the choose_cells() function from Monocle3 by the Trapnell lab
(https://cole-trapnell-lab.github.io/monocle3/). Andrew Wight was funded by an
AAI Intersect fellowship for cross-training immunologists in computational
biology.
